...
starting_nodes = [node for node in nodes.keys() if nodes[node] == 1]
end_node = (148, 20)

shortest_distances = []
for (i, node) in enumerate(starting_nodes):
    print(i)
    shortest_distances.append(BFS(node)[end_node])
...
# set that node's shortest distance to (current distance to your node) + 1.
