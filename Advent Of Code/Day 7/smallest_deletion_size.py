...
def getDirSizes(device):
    ...
    dir_vols.append(sum(device['/'].values()))
    return dir_vols

device = makeTree(commands)
dirSizes = getDirSizes(copy.copy(device))
total_space_used = max(dirSizes)
min_space_needed = 30000000
total_space_in_system = 70000000
unused_space = total_space_in_system - total_space_used
space_difference = min_space_needed - unused_space
candidate_sizes = [size for size in dirSizes if size > space_difference]
final_size = min(candidate_sizes)
print(final_size)